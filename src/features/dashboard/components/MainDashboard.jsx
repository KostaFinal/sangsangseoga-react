  ...
  return (
    <div className="min-h-screen bg-[#FAF9FF] text-[#2F2D59] font-sans pb-24">
          ...
          <div className="space-y-10 relative">
            ...
            <div className="space-y-4 pt-4 z-10 relative">
              ...
              {recentlyReadBooks.length === 0 ? (
                <p className="text-xs text-[#B9B0DC] font-bold py-6 text-center">아직 읽고 있는 작품이 없습니다</p>
              ) : (
                    ...
                    <div key={book.id} onClick={() => navigate(`/friends/${book.id}`)} className="group cursor-pointer">
                      <div className="relative w-full aspect-[3/4] rounded-xl overflow-hidden shadow-sm border border-gray-200 group-hover:shadow-md group-hover:border-[#d4cdf2] transition-all duration-300 group-hover:-translate-y-1 bg-white">
                        ...
                        <div className="absolute top-2 left-2 flex items-center gap-1.5">
                          <div className="w-6 h-6 rounded-full bg-[#6B54E7] text-white font-black text-[11px] flex items-center justify-center shadow-md border border-white/30">
                            {book.rank}
                          </div>
                          {book.bookType && (
                            <div className={`px-2 py-0.5 rounded-md text-[9px] font-bold border backdrop-blur-sm ${genreBadge(book.bookType).cls}`}>
                              {genreBadge(book.bookType).label}
                            </div>
                          )}
                        </div>
                  ...
                  {weeklyNewReleases.map((book) => (
                    <div key={book.id} onClick={() => navigate(`/friends/${book.id}`)} className="group cursor-pointer">
                      <div className="relative w-full aspect-[3/4] rounded-xl overflow-hidden shadow-sm border border-gray-200 group-hover:shadow-md group-hover:border-[#d4cdf2] transition-all duration-300 group-hover:-translate-y-1 bg-white">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-[#000000]/75 via-transparent to-transparent" />
                        <div className="absolute top-2 left-2 flex items-center gap-1.5">
                          <div className="px-2 py-0.5 rounded-md text-[9px] font-bold border backdrop-blur-sm bg-[#6b54e7]/90 text-white border-white/30">
                            NEW
                          </div>
                          {book.bookType && (
                            <div className={`px-2 py-0.5 rounded-md text-[9px] font-bold border backdrop-blur-sm ${genreBadge(book.bookType).cls}`}>
      ...
      {showFreeTrialCapModal && (
        <div className="fixed inset-0 bg-[#110F24]/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-250">
      ...
      {showAtelierSection && (
        <div className="fixed inset-0 bg-[#110F24]/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
