...
export const NotificationsView = ({ notifications, onMarkAsRead, onMarkAllAsRead }) => {
  return (
    <div className="space-y-6 animate-in fade-in duration-200 p-6 bg-white rounded-2xl border border-neutral-200/60 shadow-7xs">
      <div className="flex justify-between items-center border-b pb-4">
        ...
        <button
          ...
          className="text-sm font-bold text-neutral-500 hover:text-black uppercase tracking-widest transition-colors cursor-pointer"
        >
          모두 읽음으로 표시
        </button>
      </div>
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map((note) => (
            <button
              ...
              className={`w-full p-4 rounded-xl border flex items-start gap-4 text-left transition-colors ${note.read ? 'bg-neutral-50 border-neutral-100 cursor-default' : 'bg-white border-neutral-200 shadow-sm hover:bg-neutral-50 cursor-pointer'}`}
            >
