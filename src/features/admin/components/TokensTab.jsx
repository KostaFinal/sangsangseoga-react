  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200 text-left">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { title: '누적 텍스트 생성량', value: '48,291k', desc: '자', icon: PenTool },
          { title: '누적 이미지 생성량', value: '124.8k', desc: '장', icon: BookOpen },
          { title: '구독 회원 비율', value: '84.2%', desc: '당월 기준', icon: TrendingUp },
          { title: '무료 회원 비율', value: '15.8%', desc: '당월 기준', icon: BarChart3 }
        ].map((card, idx) => {
          ...
          return (
            <div key={idx} className="bg-white border border-[#E6E2FC]/60 rounded-2xl p-5 shadow-xs space-y-2">
              ...
              <div className="flex items-baseline space-x-1.5">
                <span className="text-xl font-mono font-black text-[#110F24]">{card.value}</span>
                <span className="text-[10px] text-[#7C769D] font-bold">{card.desc}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white border border-[#E6E2FC]/60 rounded-3xl p-6 shadow-sm space-y-6">
        <div className="flex justify-between items-center border-b border-[#E6E2FC]/30 pb-4">
          ...
          <div className="flex bg-[#FAF9FF] p-1 rounded-xl border border-[#E6E2FC]/40">
            <button
              ...
              className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${tokenTrendUnit === 'daily' ? 'bg-[#6B54E7] text-white shadow-xs' : 'text-[#7C769D] hover:text-[#110F24]'}`}
            >
              일간
            </button>
            <button
              onClick={() => setTokenTrendUnit('monthly')}
              className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${tokenTrendUnit === 'monthly' ? 'bg-[#6B54E7] text-white shadow-xs' : 'text-[#7C769D] hover:text-[#110F24]'}`}
            >
              월간
            </button>
          </div>
        ...
        <div className="relative pt-2 h-56 bg-[#FAF9FF]/50 rounded-2xl border border-[#E6E2FC]/30 p-4 flex flex-col justify-end space-y-4">
          <div className="absolute top-4 right-4 flex items-center space-x-4 text-[9px] font-bold">
            ...
            {currentTrends.map((col, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center flex-1 group relative">
                  ...
                  {/* Label */}
                  <span className="text-[10px] font-mono font-bold text-[#7C769D] mt-2 block">{col.label}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex justify-between items-center border-b border-[#E6E2FC]/30 pb-3.5">
            ...
            <div className="flex bg-[#FAF9FF] rounded-lg p-0.5 border border-[#E6E2FC]/40">
              <button 
                ...
                className={`px-2.5 py-1 text-[9.5px] font-bold rounded transition-all cursor-pointer ${tokenSortPeriod === '7d' ? 'bg-[#6B54E7] text-white shadow-xs' : 'text-[#7C769D] hover:text-[#110F24]'}`}
              >
                7일
              </button>
              <button 
                onClick={() => setTokenSortPeriod('30d')}
                className={`px-2.5 py-1 text-[9.5px] font-bold rounded transition-all cursor-pointer ${tokenSortPeriod === '30d' ? 'bg-[#6B54E7] text-white shadow-xs' : 'text-[#7C769D] hover:text-[#110F24]'}`}
              >
                30일
              </button>
            </div>
            ...
            {searchedTokenUsages.map((usage, idx) => (
              ...
              >
                <div className="flex items-center space-x-3 text-left">
                  ...
                  <div>
                    <span className="font-bold text-[#2F2D59] text-xs block">{usage.nickname}</span>
                    <span className="text-[10px] font-semibold block mt-0.5 text-[#7C769D]">{usage.plan === 'PREMIUM' ? '프리미엄' : '일반 요금제'}</span>
                  </div>
                </div>

                <div className="text-right flex items-center space-x-4">
                  <div>
                    <p className="text-xs text-[#2F2D59] font-mono font-black">
                      {(usage.textUsage / 10).toFixed(0)}k 자 <span className="text-[10px] text-[#7C769D] font-normal">({usage.imgUsage}장)</span>
                    </p>
                    <span className="text-[10px] text-[#7C769D] font-semibold block mt-0.5">누적 포인트: {usage.totalCredits.toLocaleString()}</span>
                  </div>

                  {usage.status === 'ABNORMAL' ? (
                    <span className="px-2 py-1 bg-amber-50 text-amber-700 font-extrabold border border-amber-200 rounded text-[9px] animate-pulse flex items-center gap-1 shrink-0">
                      <AlertTriangle className="w-3 h-3 text-amber-700" /> 과다 사용 의심
                    </span>
                  ) : (
                    <span className="px-2 py-1 bg-emerald-50 text-emerald-800 font-semibold border border-emerald-150 rounded text-[9px] shrink-0">
                      정상
                    </span>
                  )}
                </div>
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl border border-[#E6E2FC]/60 p-6 space-y-4">
          ...
          <div className="relative text-left">
            ...
            <input
              ...
              placeholder="작가명 검색..."
              className="w-full pl-9 pr-3 py-2 bg-[#FAF9FF] rounded-xl text-xs border border-[#E6E2FC] focus:bg-white focus:outline-none transition-all placeholder:text-[#B9B0DC] text-[#2F2D59]"
            />
          </div>
          ...
          {selectedTokenUser && memberTokenTimelineLogs[selectedTokenUser] ? (
            <div className="space-y-3 pt-2 text-left">
              <div className="flex items-center justify-between border-b border-[#E6E2FC]/30 pb-2">
                <span className="text-xs font-black text-[#2F2D59]">
                  {users.find(u => u.id === selectedTokenUser)?.nickname || '조사 대상'} 회원 사용 이력
                </span>
                <button 
                  onClick={() => setSelectedTokenUser(null)}
                  className="text-[9.5px] font-bold text-[#7C769D] hover:text-[#6B54E7] cursor-pointer"
                >
                  선택 해제
                </button>
              </div>
              ...
              <div className="relative border-l border-[#E6E2FC] pl-4.5 space-y-4 pt-1 ml-1.5">
                {memberTokenTimelineLogs[selectedTokenUser].map((log, lidx) => (
                  <div key={lidx} className="relative text-xs">
                    ...
                    <p className="text-[10px] text-[#7C769D] font-mono mt-0.5">산출량: {log.amount}</p>
                  </div>
                ))}
              </div>
            </div>
